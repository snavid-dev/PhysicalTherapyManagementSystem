				...
				<div class="tab-menu-heading border-0 p-0">
					<div class="tabs-menu1">
						...
						<ul class="nav panel-tabs product-sale" role="tablist">
							...
							<li>
								<input type="text" class="form-control" placeholder="<?= $ci->lang('select') ?>"
									   data-jdp id="fromDate">
							</li>
									...
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<button class="btn btn-warning"
													onclick="xhrSubmit('insertAccount', '<?= base_url() ?>admin/insert_receipt', 'table','extralargemodal', print_expense, false, 'print')">
					...
					<table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom">
						...
						foreach ($receipts as $receipt) : ?>
							<tr id="<?= $receipt['id'] ?>">
								...
								<td>
									<div class="g-2">
										<?php if ($ci->auth->has_permission('Update Receipt')): ?>
											<a href="javascript:edit_receipt('<?= $receipt['id'] ?>')"
											   class="btn btn-icon btn-outline-secondary rounded-pill btn-wave waves-effect waves-light"><span
													class="fa fa-edit"></span></a>
										<?php endif; ?>

										<?php if ($ci->auth->has_permission('Delete Receipt')): ?>
											<a href="javascript:delete_via_alert('<?= $receipt['id'] ?>', '<?= base_url() ?>admin/delete_receipt')"
		...
		<div class="modal-content">
					...
					<div class="row">
						...
						<div class="col-sm-12 col-md-3">
							<div class="form-group">
								...
								<select name="customers_id" class="form-control select2-show-search form-select"
										id="customers_id" data-placeholder="<?= $ci->lang('select') ?>">
									<option label="<?= $ci->lang('select') ?>"></option>
									<?php foreach ($accounts as $account) : ?>
										<option value="<?= $account['id'] ?>">
											<?= $ci->mylibrary->finance_account_name($account['name'], $account['lname'], $account['type']) ?>
						...
						<div class="col-sm-12 col-md-3">
							<div class="form-group jdp" id="update-date-div">
								...
								<input data-jdp type="text" id="update_date" name="shamsi" value="" class="form-control"
									   placeholder="<?= $ci->lang('date') ?>">
								<div></div>
							</div>
						</div>
			...
			<div class="modal-footer">
				...
				<button class="btn btn-primary"
						onclick="xhrUpdate('updateReceipt', '<?= base_url() ?>admin/update_receipt')">
					<?= $ci->lang('update') ?>
				</button>
			</div>
		</div>
				...
				if (result.content.receipts.length > 0) {
					...
					result.content.receipts.map((item) => {
						if (item.delete_access) {
							delete_button = `<a href="javascript:delete_via_alert('${item.id}', '<?= base_url() ?>admin/delete_receipt')" class="btn btn-icon btn-outline-danger rounded-pill btn-wave waves-effect waves-light"><span class="fa fa-trash fs-14"></span></a> `;
						}

						if (item.update_access) {
							update_button = `<a href="javascript:edit_receipt('${item.id}')" class="btn btn-icon btn-outline-secondary rounded-pill btn-wave waves-effect waves-light"><span class="fa fa-edit fs-14"></span></a>`;
						}
