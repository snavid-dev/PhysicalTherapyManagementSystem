<div class="d-flex justify-content-between align-items-center mb-4">
	...
	<a href="<?= base_url('leaves') ?>" class="btn btn-outline-dark btn-icon"><i class="bi bi-arrow-left icon-flip-rtl" aria-hidden="true"></i> <?= t('Back') ?></a>
</div>
...
<div class="card">
	<div class="card-body">
		<?= form_open($action) ?>
			<div class="row g-3">
				<div class="col-md-6">
					...
					<select name="staff_id" class="form-select s2-select">
						...
						<?php $selected_staff = (int) set_value('staff_id', $leave['staff_id'] ?? 0); ?>
						<?php foreach ($staff_members as $staff_member) : ?>
							<option value="<?= $staff_member['id'] ?>" <?= $selected_staff === (int) $staff_member['id'] ? 'selected' : '' ?>><?= html_escape(trim($staff_member['first_name'] . ' ' . $staff_member['last_name'])) ?><?= !empty($staff_member['staff_type_name']) ? ' (' . html_escape($staff_member['staff_type_name']) . ')' : '' ?></option>
						<?php endforeach; ?>
					</select>
			...
			<div class="mt-4">
				<button type="submit" class="btn btn-dark btn-icon"><i class="bi bi-check-lg" aria-hidden="true"></i> <?= t('Save Leave') ?></button>
			</div>
		<?= form_close() ?>
	</div>
</div>
