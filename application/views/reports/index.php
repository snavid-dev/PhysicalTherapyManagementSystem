			...
			<div class="card-body">
				...
				<ul class="list-group list-group-flush">
					<?php if ($turns) : foreach ($turns as $turn) : ?>
						<li class="list-group-item px-0"><?= html_escape($turn['turn_date']) ?> | <?= html_escape($turn['patient_first_name'] . ' ' . $turn['patient_last_name']) ?></li>
					<?php endforeach; else : ?>
			...
			<div class="card-body">
				...
				<ul class="list-group list-group-flush">
					<?php if ($payments) : foreach ($payments as $payment) : ?>
						<li class="list-group-item px-0"><?= html_escape($payment['payment_date']) ?> | <?= html_escape($payment['first_name'] . ' ' . $payment['last_name']) ?> | $<?= number_format((float) $payment['amount'], 2) ?></li>
					<?php endforeach; else : ?>
			...
			<div class="card-body">
				...
				<ul class="list-group list-group-flush">
					<?php if ($leaves) : foreach ($leaves as $leave) : ?>
						<li class="list-group-item px-0"><?= html_escape($leave['start_date']) ?> to <?= html_escape($leave['end_date']) ?> | <?= html_escape($leave['first_name'] . ' ' . $leave['last_name']) ?></li>
					<?php endforeach; else : ?>
